...
class WebPayNormal {
    ...
    public function initTransaction($amount, $buyOrder, $sessionId , $urlReturn, $urlFinal) {
        ...
        try {
		    ...
		    $temporal = "log.txt";
			if($fp = fopen($temporal, "a")) {
				fwrite($fp, "\n\n initTransaction: \n");
				fwrite($fp, "REQUEST\n");
				$r = var_export($wsInitTransactionInput, true);
				fwrite($fp, "$r \n");
				fwrite($fp, "RESPONSE\n");
				$v = var_export($initTransactionResponse->return, true);
				fwrite($fp, "$v \n");				
				fclose($fp);
            }
    ...
    public function getTransactionResult($token) {
	    ...
	    $temporal = "log.txt";
			if($fp = fopen($temporal, "a")) {
				...
				fwrite($fp, "REQUEST\n");
				$r = var_export($getTransactionResult , true);
				fwrite($fp, "$r \n");
				fwrite($fp, "RESPONSE\n");
				$v = var_export($getTransactionResultResponse->return, true);
				fwrite($fp, "$v \n");
				fclose($fp);
            }
    ...
    public function acknowledgeTransaction($token) {
	 ...
	 $temporal = "log.txt";
			if($fp = fopen($temporal, "a")) {
				...
				fwrite($fp, "REQUEST\n");
				$r = var_export($acknowledgeTransaction, true);
				fwrite($fp, "$r \n");
				fwrite($fp, "RESPONSE\n");
				$v = var_export( $validationResult, true);
				fwrite($fp, "$v \n");
				fclose($fp);
            } 
